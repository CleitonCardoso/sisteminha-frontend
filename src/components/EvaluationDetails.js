...
const avaliacoesService = new EvaluationsService()
...
export default class EvaluationDetails extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      evaluation: {
  ...
  handleSave = () => {
    avaliacoesService.save(response => {
      if (response.status === 200) {
        this.props.history.push('/INCUBATOR/avaliacoes')
      }
    }, this.state.evaluation)
  }
